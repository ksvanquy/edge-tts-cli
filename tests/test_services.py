...
def make_service(retries: int = 1) -> TTSService:
    return TTSService(AsyncMock(), make_config(retries))
...
def test_synthesize_with_retry_recovers_after_failure(tmp_path: Path, monkeypatch):
    ...
    service.engine.synthesize = AsyncMock(side_effect=[RuntimeError("network"), cues])
...
def test_generate_dry_run_does_not_create_project_artifacts(tmp_path: Path, capsys):
    number = asyncio.run(
        make_service(retries=0).generate(
...
def test_generate_cleans_new_folder_when_synthesis_fails(tmp_path: Path):
    ...
    async def fail_synthesize(text: str, audio_path: Path) -> list[SubtitleCue]:
        audio_path.write_bytes(b"partial")
    ...
    with pytest.raises(RuntimeError, match="synthesis failed"):
        asyncio.run(service.generate("Xin", tmp_path, "phrase", 8))
